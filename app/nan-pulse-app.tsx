...
function AdaptiveExperiencePlanner() {
  ...
  const choose = (nextInterest:string, nextCondition:string) => { setInterest(nextInterest); setCondition(nextCondition); setPlanned(true); };
  return <div className="product-workspace">
    ...
    <section className="planner-hero">
      <div className="planner-question"><p className="eyebrow">{tr("เลือกข้อมูลแล้วดู AI ปรับทันที", "Change inputs and watch AI adapt")}</p><h2>{tr("เปลี่ยนสัญญาณ", "Change the signals")}<br />{tr("เปลี่ยนเส้นทาง", "Change the journey")}</h2><div className="interest-pills">{plannerProfiles.map(item => <button key={item.id} className={interest === item.id ? "active" : ""} onClick={() => { setInterest(item.id); setPlanned(true); }}>{item.icon} {item.id}</button>)}</div><div className="condition-pills">{plannerConditions.map(item => <button key={item.id} className={condition === item.id ? "active" : ""} onClick={() => { setCondition(item.id); setPlanned(true); }}><i>{item.icon}</i>{tr(item.labelTh,item.labelEn)}</button>)}</div><button className="primary-action" onClick={() => setPlanned(true)}>{planned ? tr("AI ปรับเส้นทางแล้ว ✓", "Journey adapted ✓") : tr("ให้ AI ปรับเส้นทาง", "Adapt journey with AI")}</button></div>
      <div key={condition} className="adaptive-signals signal-change"><p className="eyebrow">Live conditions · {live.icon}</p><div><span>Weather</span><strong>{live.weather}</strong><small>{live.swap}</small></div><div><span>Season</span><strong>{live.season}</strong><small>{tr("ตรวจสอบหน้าต่างโอกาสแล้ว", "Opportunity window checked")}</small></div><div><span>Tourism pulse</span><strong>{live.density}</strong><small>{live.reason}</small></div><div className="signal-risk"><span>Route risk</span><strong>{live.risk}</strong><small>{tr("AI ประเมินก่อนเปลี่ยนเส้นทาง", "AI evaluated before rerouting")}</small></div></div>
    </section>
    <section key={`${interest}-${condition}`} className={`experience-plan ${planned ? "ready" : ""}`}>
    ...
    <section key={`change-${interest}-${condition}`} className="planner-change-proof"><div className="change-proof-head"><div><p className="eyebrow">{tr("หลักฐานการเปลี่ยนแปลง", "Decision Change Evidence")}</p><h2>{tr("AI เปลี่ยนอะไรจากเส้นทางเดิม", "What AI changed from the original route")}</h2></div><span>{confidence}% AI confidence</span></div><div className="journey-before-after"><article><p>{tr("ก่อน AI", "Before AI")}</p><strong>{profile.from}</strong><small>{tr("เส้นทางยอดนิยม · ความหนาแน่นสูง", "Popular route · high density")}</small><i><b style={{width:"82%"}} /></i><em>82% density</em></article><b>→</b><article className="after"><p>{tr("หลัง AI", "After AI")}</p><strong>{profile.to}</strong><small>{live.reason}</small><i><b style={{width:`${28 + live.visitors / 2}%`}} /></i><em>{28 + Math.round(live.visitors / 2)}% density</em></article></div><div className="journey-deltas"><span><b>+{live.visitors}</b>{tr("นักท่องเที่ยวถูกกระจาย", "travelers redirected")}</span><span><b>+฿{expectedIncome.toLocaleString()}</b>{tr("มูลค่าสู่ท้องถิ่น", "local value")}</span><span><b>{profile.communities}</b>{tr("ชุมชนได้รับประโยชน์", "communities benefit")}</span><span><b>{live.swap}</b>{tr("การเปลี่ยนที่ AI ทำ", "AI route change")}</span></div><p className="planner-mock-note">{tr("ข้อมูลจำลองสำหรับสาธิต · เลือกสถานการณ์ด้านบนเพื่อดูเส้นทาง สัญญาณ เหตุผล และผลกระทบเปลี่ยนพร้อมกัน", "Demonstration mock data · Select a scenario above to see route, signals, reasoning, and impact change together.")}</p></section>
  </div>;
}
