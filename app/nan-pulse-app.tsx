...
function CommunityCopilot() {
  ...
  return <div className="product-workspace">
    ...
    <section className="copilot-example-library" aria-labelledby="copilot-examples-title">
      <div className="copilot-example-head"><div><p className="eyebrow">{tr("คลังตัวอย่างจำลอง", "Mock Example Library")}</p><h2 id="copilot-examples-title">{tr("12 ภารกิจที่ให้ผลลัพธ์ต่างกัน", "12 missions with distinct outputs")}</h2></div><span>{tr("เลือกตัวอย่างเพื่อทดลองทันที", "Select an example to try it instantly")}</span></div>
      <div className="copilot-example-grid">{copilotScenarios.map((item, index) => <button key={`${item.community}-${item.need}`} className={active === item ? "active" : ""} onClick={() => selectExample(item, index)}><span>{item.community}</span><strong>{communityNeeds.find(need => need.id === item.need)?.[locale === "th" ? "th" : "en"]}</strong><small>{item.campaign}</small><em>{item.confidence}%</em></button>)}</div>
    </section>
    <section className="copilot-layout">
      <div className="copilot-input">
        ...
        <div className="community-asks" role="group" aria-label="Community missions">{communityNeeds.map(need => <button key={need.id} className={mission === need.id ? "active" : ""} onClick={() => { setMission(need.id); setCreated(true); }}>{locale === "th" ? need.th : need.en}</button>)}</div>
        <label>{tr("ชุมชน", "Community")}<select value={community} onChange={e => { setCommunity(e.target.value); setCreated(true); }}><option>เวียงสา</option><option>สันติสุข</option><option>แม่จริม</option></select></label>
      ...
      <div key={`${community}-${mission}-${goal}`} className={`copilot-output ${created ? "ready" : ""}`}>
        ...
        <div className="content-pack"><article><span>{tr("โปรโมชัน", "Promotion")}</span><strong>{locale === "th" ? active.promotionTh : active.promotionEn}</strong></article><article><span>{tr("คำบรรยายภาษาไทย", "Thai caption")}</span><p>{active.captionTh}</p></article><article><span>{tr("คำบรรยายภาษาอังกฤษ", "English caption")}</span><p>{active.captionEn}</p></article><article><span>{tr("กิจกรรมแนะนำ", "Suggested activity")}</span><strong>{active.activity}</strong></article></div>
        <div className="output-impact"><span>{tr("ผลลัพธ์ที่คาดการณ์", "Expected result")}</span><strong>+{expectedVisitors} {tr("คน", "visitors")}</strong><strong>+{expectedIncome.toLocaleString()} {tr("บาท", "THB")}</strong><strong>{active.households} {tr("ครัวเรือน", "households")}</strong></div>
